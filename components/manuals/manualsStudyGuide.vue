		...
		>
			<template v-if="isNotes === true">
				<div v-for="manual in subtopic.manuals" :key="manual._id" class="mb-3" :class="{'review': manual.finished}">
					<nuxt-link class="pointer body-medium-3" :to="`/review?manual_id=${manual.id}`" v-if="manual.finished">
						{{ manual.name }}
					</nuxt-link>
					<div class="body-medium-3" style="opacity: 0.30" v-else>
						{{ manual.name }}
					</div>
				</div>
			</template>
			<template v-else>
				<div v-for="manual in subtopic.manuals" :key="manual._id" class="mb-3">
					<div v-if="manual.isExtra">
						<nuxt-link class="pointer body-medium-3" :to="`/manual?manual_id=${manual.id}&extra=true&finishManualExtra=${manual.finished}`">
							{{ manual.name }}
						</nuxt-link>
					</div>
					<div v-else>
						<nuxt-link class="pointer body-medium-3" :to="`/manual?manual_id=${manual.id}`">
							{{ manual.name }}
						</nuxt-link>
					</div>
				</div>
