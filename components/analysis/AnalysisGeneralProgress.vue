<template>
  <article ref="my-general-progress">
    ...
    >
      <div
        v-for="manual in topics"
        :key="manual._id"
        class="topic-container text-center"
      >
        <div
          class="
            chart-container
          ...
          "
          :style="{
            width: `${doughnutWidthContainer}px`,
            height: `${doughnutWidthContainer}px`,
          }"
          :set="
            (topicCalculations = calculePercentage(
              manual.total,
              manual.progress,
            ))
          "
        >
          ...
          <div class="caption-progress body-xxlg2">
            {{ topicCalculations[0] }} %
          </div>
        </div>
        <h4>{{ manual.name }}</h4>
      ...
      <div class="topic-container text-center">
        <div
          ...
          :set="
            (topicCalculations = calculePercentage(totalManuals, totalProgress))
          "
        >
          <div class="doughnut-chart w-100 w-100">
            <doughnut-chart
...
export default {
      ...
      tooltips: {
        ...
        callbacks: {
          ...
          label: (tooltipItem, data) => {
            ...
            const tag =
              data.datasets[0].data[indice] === 1 ? 'manual' : 'manuales';
  ...
  mounted () {
    ...
    this.doughnutWidthContainer = this.calculeChartSize(
      containerWidth,
      nElements,
      1,
    );
