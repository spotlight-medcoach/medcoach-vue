    ...
    <div class="controls d-flex justify-content-between">
      ...
      <b-dropdown size="sm" variant="white" no-caret>
        <template #button-content>
          <span class="body-title-2 mr-2">{{
            selectedScope.toFormat('MMMM yyyy')
          }}</span>
        ...
        <b-dropdown-item-button
          v-for="(scope, index) in montScopeOptions"
          :key="index"
          @click="selectedScope = scope"
        >
          <b-icon
            :icon="selectedScope === scope ? 'check' : 'blank'"
            aria-hidden="true"
          />
          <span v-if="scope" class="body-title-5">{{
            scope.toFormat('MMMM yyyy')
          }}</span>
        </b-dropdown-item-button>
      </b-dropdown>
    </div>
...
import {
  fillEmptyDaysGroupingByMonth,
  formatByDayGroupingByMonth,
} from './analysisStudytimeChartCalculations.js';
...
export default {
  ...
  data: () => ({
    ...
    layerMap: [
      { title: 'Mis horas de estudio', color: '#FF9300' },
      {
        title: 'Horas de estudio promedio de todos los usuarios',
        color: '#818FF9',
      },
    ],
    options: {
  ...
  methods: {
    ...
    calculeStudentSubscriptionMonths () {
      if (this.student && this.subscriptionMonths == null) {
        const subcriptionStartAt = DateTime.fromISO(this.student.paid_at, {
          zone: 'utc',
        });
        const subcriptionEndAt = DateTime.fromISO(this.student.end_date, {
          zone: 'utc',
        });
        this.subscriptionMonths = fillEmptyDaysGroupingByMonth(
          subcriptionStartAt,
          subcriptionEndAt,
        );
        this.montScopeOptions = Object.values(this.subscriptionMonths).map(
          (sm) => sm.date,
        );
      }
    },
    calculeGlobalStudyTimesDaysGroupedByMonth () {
      if (
        ...
        this.globalStudyTimesDaysGroupedByMonth == null
      ) {
        this.globalStudyTimesDaysGroupedByMonth = formatByDayGroupingByMonth(
          this.communityStats.days,
        );
      }
    },
    calculeStudyTimesDaysGroupedByMonth () {
      if (
        this.studyTime &&
        this.subscriptionMonths &&
        this.studyTimesDaysGroupedByMonth == null
      ) {
        this.studyTimesDaysGroupedByMonth = formatByDayGroupingByMonth(
          this.studyTime.days,
        );
      }
    },
    calculeStudentCurrentMonth () {
      if (
        this.studyTimesDaysGroupedByMonth &&
        this.globalStudyTimesDaysGroupedByMonth
      ) {
        const selectedScopeDataStudent = this.calculeDatasetByMonth(
          this.selectedScope.toFormat('yyyyMM'),
          this.studyTimesDaysGroupedByMonth,
        );
        const selectedScopeDataGlobal = this.calculeDatasetByMonth(
          this.selectedScope.toFormat('yyyyMM'),
          this.globalStudyTimesDaysGroupedByMonth,
        );
    ...
    calculeDatasetByMonth (monthKey, studyTimesDaysGroupedByMonth) {
      ...
      if (studyTimeMonthRef) {
        // Sólo si hay registros de studyTimesDaysGroupedByMonth en el mes scope
        const mergedDays = { ...emptyMonthRef.days, ...studyTimeMonthRef.days };
        scopedMonthRef = Object.assign({}, studyTimeMonthRef, {
          days: mergedDays,
        });
      } // De lo contrario se envía vacío
      const title = DateTime.fromISO(scopedMonthRef.start_on).toFormat(
        'MMMM yyyy',
      );
    ...
    fillChart (data) {
      ...
      this.chartDay.datasets = data.map(([title, labels, values], idx) => {
        this.chartDay.labels =
          this.chartDay.labels > labels ? this.chartDay.labels : labels;
