<template>
  <article
    v-if="studentStats"
    class="d-flex flex-row justify-content-between align-items-center ml-5"
  >
            ...
            <doughnut-chart
              :chart-data="{
                ...
                datasets: [
                  {
                    data: [
                      studentStats.total_correct_questions,
                      studentStats.total_wrong_questions,
                    ],
            ...
            <doughnut-chart
              :chart-data="{
                ...
                datasets: [
                  {
                    data: [
                      studentStats.total_used_questions,
                      studentStats.total - studentStats.total_used_questions,
                    ],
...
const defaultFields = [
  {
    ...
    tdClass: "body-lg-text-light text-left",
  },
  {
    key: "results",
    label: "",
    thClass: "body-title-2 pb-3",
    tdClass: "body-lg-text-light text-right",
  },
];
export default {
  components: {
      ...
      tooltips: {
        ...
        callbacks: {
          ...
          label: (tooltipItem, data) => {
            ...
            const tag =
              data.datasets[0].data[indice] === 1 ? "pregunta" : "preguntas";
  ...
  computed: {
        ...
        items: [
          {
            ...
            results: this.studentStats
              ? this.studentStats.total_correct_questions
              : this.placeholderText,
          },
          {
            approach: "Total incorrectas",
            results: this.studentStats
              ? this.studentStats.total_wrong_questions
              : this.placeholderText,
          },
        ],
      };
        ...
        items: [
          {
            ...
            results: this.studentStats
              ? this.studentStats.total_used_questions
              : this.placeholderText,
          },
          {
            approach: "Preguntas sin utilizar",
            results: this.studentStats
              ? this.studentStats.total - this.studentStats.total_used_questions
              : this.placeholderText,
          },
          {
        ...
        items: [
          {
            ...
            results: this.studentStats
              ? this.studentStats.change_correct_to_incorrect
              : this.placeholderText,
          },
          {
            approach: "Incorrecta a correcta",
            results: this.studentStats
              ? this.studentStats.change_incorrect_to_correct
              : this.placeholderText,
          },
          {
            approach: "Incorrecta a incorrecta",
            results: this.studentStats
              ? this.studentStats.change_incorrect_to_incorrect
              : this.placeholderText,
          },
        ],
      };
        ...
        items: [
          {
            ...
            results: this.studentCustomTests
              ? this.studentCustomTests.length
              : this.placeholderText,
          },
          {
            approach: "Exámenes completados",
            results: this.finishedCustomTests
              ? this.finishedCustomTests
              : this.placeholderText,
          },
          {
            approach: "Exámenes suspendidos",
            results: this.discontinuedCustomTests
              ? this.discontinuedCustomTests
              : this.placeholderText,
          },
        ],
      };
    ...
    finishedCustomTests() {
      if (this.studentCustomTests) {
        return this.studentCustomTests.reduce(
          (prev, curr) => (curr.finished ? prev + 1 : prev),
          0
        );
      }
      return 0;
  ...
  methods: {
    calculePercentage(totalValue, actualValue) {
      if (totalValue !== 0 && actualValue !== 0) {
        const learned = +parseFloat((actualValue / totalValue) * 100).toFixed(
          2
        );
