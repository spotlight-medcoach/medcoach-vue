    ...
    chartOptions: {
      ...
      tooltips: {
        ...
        callbacks: {
          title: () => {},
          label: (tooltipItems) => {
            return tooltipItems.yLabel === -25
              ? 'Pendiente'
              : 'Calificación: ' + tooltipItems.yLabel;
          },
        },
      },
      layout: {
      ...
      if (this.studentCustomTests) {
        const studentCustomTests = structuredClone(this.studentCustomTests)
          ...
          .reduce((acc, customTests) => {
            const ISODate = DateTime.fromFormat(customTests.date, 'DDD', {
              locale: 'es',
            }).toFormat('dd/MM');
            const cleanScoreString = customTests.score.replace(/[^\d.]/g, '');
            const score = cleanScoreString
              ? +parseFloat(cleanScoreString).toFixed(2)
              : 0;
            const background = customTests.finished ? '#FF930080' : '#BBBBB380';
            acc.labels.push(
              customTests.finished ? `${ISODate}` : `• ${ISODate}`,
            );
        ...
        if (this.studentCustomTests.length < this.minChartBarColumns) {
          const nEmptyCols =
            this.minChartBarColumns - this.studentCustomTests.length;
