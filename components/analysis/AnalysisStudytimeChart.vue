...
<template>
  <article>
    <h2 class="heading-6">Mi estudio total</h2>
...
export default {
  ...
  data: () => ({
    ...
    layerMap: [
      { title: 'Current', color: '#FF9300' },
      { title: 'Best', color: '#818FF9' },
    ],
    chartOptions: {
  ...
  methods: {
    ...
    calculeStudyTimesDaysGroupedByMonth () {
      if (
        ...
        this.bestScopeData = this.calculeDatasetByMonth(
          this.studyTimesDaysGroupedByMonth.best.key,
        );
      }
    },
    calculeStudentCurrentMonth () {
      ...
      if (
        ...
        const selectedScopeData = this.calculeDatasetByMonth(
          this.selectedScope.toFormat('yyyyMM'),
        );
