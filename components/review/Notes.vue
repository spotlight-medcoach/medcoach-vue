    ...
    >
      <template v-slot:modal-title>
        ...
        <div v-else-if="showDeleteAlert" class="delete-flashcard-alert">
          <p class="font-italic">
            ¿Deseas eliminar esta flashcard? Esta acción no puede deshacerse.
          </p>
          <div class="d-flex justify-content-between align-items-center">
            ...
            >
              <b-icon
                ...
                class="mr-1 h5 mb-0"
              />
              <span style="font-size: 16px" class="text-danger">Eliminar</span>
            </p>
          </div>
        </div>
      </template>
      <div v-if="showSuccessAlert" class="w-100 text-center mb-3">
        <b-icon icon="check-circle-fill" variant="success" class="my-3 h1" />
      ...
      <div v-else class="w-100 d-flex position-relative">
          ...
          <vue-flip v-model="flipped" width="100%" height="100%">
              ...
              <div class="tarjeta frente" v-if="flashcard">
                ...
                <div v-else @click="flipped = !flipped">
                  <div class="d-flex mb-1 justify-content-end">
                    ...
                    <div
                      class="h4"
                      @click.stop.prevent="showDeleteAlert = !showDeleteAlert"
                    >
            ...
            <template v-slot:back class="back">
              <div class="tarjeta detras" v-if="flashcard">
                ...
                <div v-else @click="flipped = !flipped">
                  <div
                    class="h4 mb-2 text-right"
                    @click.stop.prevent="openQuillB"
                  >
                    <b-icon icon="pencil-square" />
                  </div>
                  <div
        ...
        <div
          ...
          class="h2 mb-0 chevron"
          :class="{
            'disabled-chevron': flashcards_index >= flashcards.length - 1,
          }"
          @click="nextFlashcard"
        >
...
export default {
  ...
  computed: {
    ...
    manual () {
      const manual = this.$store.state.topics.manuals.find(
        (element) => element.manual_id === this.manual_id,
      );
      return manual;
    },
    ...mapGetters({
