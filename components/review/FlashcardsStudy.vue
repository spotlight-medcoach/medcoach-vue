<template>
  <section id="flashcards-study">
    ...
    <div id="flashcard-wrapper" v-if="flashcard">
      ...
      <!-- Icono de Edición ---------------->
      <span class="option-icon edit-icon" @click="goToForm">
        <EditIcon />
      </span>
      ...
      <div class="d-flex align-items-center justify-content-between">
        <div class="d-flex align-items-center">
          <span
            @click="
              $store.commit('flashcards/setRepeatActivated', !repeatActivated)
            "
          ...
          <span
            @click="
              $store.commit('flashcards/setRandomActivated', !randomActivated)
            "
            class="option-icon"
            :class="{
        ...
        <div class="d-flex align-items-center">
          <span @click="deleteFlashcard" class="option-icon">
            <TrashIcon
              :style="{
                height: '30px',
              }"
      ...
      <div class="flashcards-study-menu mr-3">
...
export default {
  ...
  computed: {
    ...
    nextBtnActive() {
      return (
        this.repeatActivated ||
        this.flashcard_index < this.flashcards.length - 1
      );
    },
  },
  watch: {
  ...
  methods: {
    prevFlashcard() {
      if (this.prevBtnActive) {
        this.flashcard_index =
          this.repeatActivated && this.flashcard_index === 0
            ? this.flashcards.length - 1
            : this.flashcard_index - 1;
      }
    },
    nextFlashcard() {
