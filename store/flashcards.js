...
export const getters = {
	...
	selectedFlashcard: state => state.flashcards.find(flashcard => flashcard._id === state.selected_flashcard_id),
...
export const actions = {
	...
	async deleteFlashcard ({ commit, dispatch, state }, flashcardId) {
		try {
			const { data } = await this.$axios.delete('manuals/flashcard', {
				data: {
			...
			console.error(error)
			this._vm.$toastr.error('Hubo un problema al eliminar su flashcard', 'Error')
		}
	},
	async updateFlashcard ({ commit, dispatch, state }, formData) {
		try {
			...
			console.error(error)
			this._vm.$toastr.error('Hubo un problema al guardar su flashcard', 'Error')
		}
	},
