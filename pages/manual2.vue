<template>
  <div id="manual" :style="{ 'background-color': actualColorBg }">
    ...
    <b-navbar
      :style="{
        'background-color': `${actualColorBg} !important`,
        color: `${actualColorFont} !important`,
      }"
      class="navbarBg"
      toggleable="lg"
      type="dark"
    ...
    >
      ...
      <b-collapse id="nav-collapse" is-nav>
        ...
        <b-navbar-nav class="ml-auto">
          ...
          <b-button
            :style="{ 'background-color': actualColorBtn }"
            size="sm"
            class="btnColorChange mr-2"
            type="submit"
            @click="onchangeColorBg"
          />
          <b-button
            v-if="
              !showLoading &&
  ...
  methods: {
    ...
    optionClicked (event) {
      if (event.option.id === 1) {
        ...
        setTimeout(() => {
          this.$refs.flashBQuillEditor.quill.clipboard.dangerouslyPasteHTML(
            this.html_selection,
          );
        }, 500);
      }
    ...
    async beforeLeave () {
      if (!this.savedNotes) {
        this.$toastr.success(
          'Guardando las Notas antes de salir',
          'Espere un momento',
        );
        const params = {
          manual_id: this.manual_id,
