<template>
  <div
    id="diagnostic_test_new"
    class="d-flex mx-0 justify-content-center h-100"
  >
    <div v-if="question" class="d-flex">
      <div id="right-side-questions">
        ...
        >
          ...
          <ToggleSwitch v-model="question.marked" />
        </section>
        <!-- Descripción del caso clínico -->
        <section id="question-content" class="custom-card">
          <div id="case-question">
            <div id="case-description">
              <p class="body-title-1 mb-10px">
    ...
    <div v-else>
      <loading-state
        :message="
          polling
            ? 'Preparando examen, por favor espere...'
...
export default {
  ...
  data () {
    return {
      ...
      maxPollingTime: 60000, // 60 seconds
    };
  },
  computed: {
    question () {
  ...
  methods: {
    ...
    startPolling () {
      ...
      // Set timeout to stop polling after maxPollingTime
      this.pollingTimeout = setTimeout(() => {
        this.stopPolling();
        this.error_request = true;
        this.message_error =
          'El examen está tardando más de lo esperado. Por favor recarga la página.';
      }, this.maxPollingTime);
      ...
      this.pollingInterval = setInterval(async () => {
        try {
          ...
          // Still 404, continue polling
          if (e.response && e.response.status === 404) {
            console.log('Esperando creación del examen...');
          } else {
    ...
    setSizes () {
      ...
      const rightSideQuestions = document.getElementById(
