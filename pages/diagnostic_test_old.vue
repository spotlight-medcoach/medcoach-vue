<template>
  <div
    id="diagnostic_test"
    :style="{ 'background-color': $store.state.themes.actualColorBg }"
  >
    <!------   HEADER    ----->
    <b-navbar
      :style="{
        'background-color': `${$store.state.themes.actualColorBg} !important`,
        color: `${$store.state.themes.actualColorFont} !important`,
      }"
      class="navbarBg pl-0"
      toggleable="lg"
      type="dark"
    ...
    >
      <b-navbar-nav style="width: 8.333333%" class="mr-5">
        <div
          :style="`font-size:${$store.state.themes.fontSize}em; color: ${$store.state.themes.actualColorFont}`"
          class="center"
        >
      ...
      <b-navbar-nav>
        <!-- BACK -->
        <div
          class="mr-5 pointer text-center"
          :class="{ disabled: question_index === 0 }"
          @click="backQuestion"
        >
        ...
        <!-- NEXT -->
        <div
          class="ml-5 pointer text-center"
          :class="{ disabled: question_index === questions.length - 1 }"
          @click="nextQuestion"
        >
      ...
      <b-collapse id="nav-collapse" is-nav>
        ...
        <b-navbar-nav class="ml-auto">
          ...
          <b-button
            :style="{ 'background-color': $store.state.themes.actualColorBtn }"
            size="sm"
            class="btnColorChange mr-2"
            type="submit"
            @click="$store.dispatch('themes/changeThemeColor')"
          />
        </b-navbar-nav>
      </b-collapse>
    </b-navbar>
      ...
      <b-row class="m-0" style="height: 100%">
        ...
        <b-col id="questions" ref="questions" cols="1" class="p-0 scroll">
          ...
          >
            ...
            <div
              class="flag d-flex align-items-center"
              @click="toggleFlag(index)"
            >
              <div v-show="quest.flag">
                <img
                  v-if="themeColor === 'light'"
                  src="@/assets/icons/orange_flag.svg"
                  alt=""
                />
                <img
                  v-else-if="themeColor === 'sepia' || question_index === index"
    ...
    if (!token) {
      ...
      try {
        ...
        let answers = JSON.parse(
          localStorage.getItem('diagnostic_test_answers'),
        );
        if (answers === null) {
    ...
    finishTest () {
      ...
      this.$axios
        ...
        .catch((error) => {
          ...
          this.message_error =
            'Lo sentimos. No se pudieron enviar las respuestas';
        });
    },
  },
};
